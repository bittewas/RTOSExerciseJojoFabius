...
#define configSTACK_OVERHEAD_TOTAL                                             \
  (STACK_OVERHEAD_CHECKER + STACK_OVERHEAD_OPTIMIZATION +                      \
   STACK_OVERHEAD_APPTRACE + STACK_OVERHEAD_WATCHPOINT)
 ...
 * - Keep this section in-sync with the corresponding version of single-core
 * upstream version of FreeRTOS
 * - Don't put any Amazon SMP FreeRTOS or IDF FreeRTOS configurations here.
 * Those go into the next section
 * - Not all FreeRTOS configuration are listed. Some configurations have default
 * values set in FreeRTOS.h thus don't need to be explicitly defined.
 * ------------------------------------------------------------------------------------------------------------------
 */
...
#define configKERNEL_INTERRUPT_PRIORITY                                        \
  1 /*Todo: This currently isn't used anywhere */
...
#define configRECORD_STACK_HIGH_ADDRESS                                        \
  1 /* This must be set as the port requires TCB.pxEndOfStack */
...
#define configTIMER_SERVICE_TASK_CORE_AFFINITY                                 \
...
#define configLIST_VOLATILE                                                    \
  volatile /* We define List elements as volatile to prevent the compiler from \
              optimizing out essential code */
...
/*
 * For trace macros.
 * Note: Include trace macros here and not above as trace macros are dependent
 * on some of the FreeRTOS configs
 */
#ifndef __ASSEMBLER__
...
/* portNUM_PROCESSORS is deprecated and will be removed in ESP-IDF v6.0
 * (IDF-8785) Please use the Kconfig option CONFIG_FREERTOS_NUMBER_OF_CORES
 * instead.
 */
#define portNUM_PROCESSORS configNUMBER_OF_CORES
// void traceQueueSendingTask(QueueHandle_t *pxQueue) { return; }

#include "QueueTraceMacros.h"
#include "TaskTraceMacros.h"
#include "TickTraceMacros.h"
